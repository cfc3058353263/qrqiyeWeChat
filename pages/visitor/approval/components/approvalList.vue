<template>
	<mescroll-uni :fixed="false" top="20" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" style="padding: 0 20rpx;"
		:down="downOption" :up="upOption">
		<view class="card" v-for="(item,index) of dataList" :key="index" @click="link(item)">
			<view class="header">
				<view class="title">{{item.tittle}}</view>
				<u-tag v-if="item.status == '1'" :text="recordStatus[item.status]" plain size="mini" type="primary"></u-tag>
				<u-tag v-else-if="item.status == '2'" :text="recordStatus[item.status]" plain size="mini" type="success"></u-tag>
				<u-tag v-else-if="item.status == '3'" :text="recordStatus[item.status]" plain size="mini" type="error"></u-tag>
				<u-tag v-else-if="item.status == '4'" :text="recordStatus[item.status]" plain size="mini" type="warning"></u-tag>
			</view>
			<view class="body">
				<view class="item">
...
<script>
	...
	export default {
		...
		methods: {
			...
			link(item) {
				this.$Router.push({
					path: "/pages/visitor/approval/details",
					query: {
						id: item.id,
						status: this.status
					}
