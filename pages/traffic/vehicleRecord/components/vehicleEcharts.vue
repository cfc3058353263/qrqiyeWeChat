...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.getByInOut(null,null,15) //流量
		},
		methods: {
			//
				...
				getByDept({
					...
					setTimeout(() => {
						...
						res.data.data.map(item => {
							...
							item['value'] = (+item.deptCount)
							categories.push(item.department)
							lineData.push(+item.deptCount)
						})
			...
			getByInOut(startTime, endTime, days) {
				getByInOut({
					startTime: startTime,
					endTime: endTime,
					days: days
			...
			change(e) {
				...
				this.getByPosiIn(1) //门禁统计出
				this.getByDept()
				this.getByInOut(this.startTime, this.endTime, null)
			},
			confirm(e) {
				this.days = e.value[0].label
				this.showTime = false
				if (e.value[0].value) {
					this.getByInOut(null, null, e.value[0].value)
				}
			}
		}
	};
