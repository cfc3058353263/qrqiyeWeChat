<template>
	<!-- 车牌组件 -->
	<u-overlay :show="plateShow" @click="close">
		<view class="plate-content">
			<view class="plate-popup" v-show="plateShow">
				...
				<view class="plate-close" @click.stop="close">关闭</view>
				<view v-show="!plateNumber">
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" v-for="(item,itemIndex) in keyVehicle1"
							:key="itemIndex+ '-only'" @click.stop="plateHead(item)">{{item}}</view>
					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" v-for="(item,itemIndex) in keyVehicle2"
							:key="itemIndex+ '-only'" @click.stop="plateHead(item)">{{item}}</view>
					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" v-for="(item,itemIndex) in keyVehicle3"
							:key="itemIndex+ '-only'" @click.stop="plateHead(item)">{{item}}</view>
					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" v-for="(item,itemIndex) in keyVehicle4"
							:key="itemIndex+ '-only'" @click.stop="plateHead(item)">{{item}}</view>
						<!-- 删除 -->
						<view class="plate-popup-item-list del" @click.stop="plateDel">
							<image :src="src" />
						</view>
					</view>
				</view>
				...
				<view v-show="plateNumber">
					<view class="plate-popup-item">
						...
						<view class="plate-popup-item-list" :class="(plateNumber.length<2 || isInputZh)?'':''"
							v-for="(item,itemIndex) in keyNumber" :key="itemIndex" @click.stop="plateNum(item)">{{item}}
						</view>

					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" :class="!isInputZh?'':''"
							v-for="(item,itemIndex) in keyEnInput1" :key="itemIndex+ '-only'" @click.stop="plateInput(item)">
							{{item}}
						</view>
					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" :class="!isInputZh?'':''"
							v-for="(item,itemIndex) in keyEnInput2" :key="itemIndex+ '-only'" @click.stop="plateInput(item)">
							{{item}}
						</view>
					</view>
					<view class="plate-popup-item">
						<view class="plate-popup-item-list" :class="!isInputZh?'':''"
							v-for="(item,itemIndex) in keyEnInput3" :key="itemIndex+ '-only'" @click.stop="plateInput(item)">
							{{item}}
						</view>
						<!-- 车牌最后一位-->
						<view class="plate-popup-item-list"
							:class="(plateNumber.length===6 || plateNumber.length===7) &&!isInputZh?'':''"
							v-for="(item,itemIndex) in keyEnInput4" :key="itemIndex" @click.stop="plateLast(item)">{{item}}
						</view>
						<!-- 删除 -->
						<view class="plate-popup-item-list del" @click.stop="plateDel">
							<image :src="src" />
